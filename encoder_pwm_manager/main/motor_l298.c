...
{
    // Intentamos de más fino a más tosco (13->10 bits)
...
esp_err_t motor_l298_init(int gpio_in1, int gpio_in2, int gpio_ena_pwm, uint32_t pwm_freq_hz)
{
    ...
    // LEDC timer con fallback (esto arregla tu crash de 20kHz+13bits)
    ...
    // Cola: tamaño 1 (si mandas mil comandos, solo manda el último)
...
esp_err_t motor_l298_set(float cmd)
{
    ...
    // xQueueOverwrite solo vale para colas de tamaño 1 (y aquí sí lo es)
